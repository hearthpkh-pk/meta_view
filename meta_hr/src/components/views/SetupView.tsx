...
export default function SetupView({ showNotification }: { showNotification: (msg: string, type?: 'success' | 'error') => void }) {
    ...
    const movePageOrder = (index: number, direction: 'up' | 'down') => {
        ...
        if (direction === 'up' && index > 0) {
            const temp = newPages[index].order_index;
            newPages[index].order_index = newPages[index - 1].order_index;
            newPages[index - 1].order_index = temp;
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in duration-300">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden h-fit">
                <div className="p-5 border-b border-slate-100 flex flex-col items-start">
                            ...
                            {sortedPages.map((page, index) => (
                                <div key={page.id} className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl p-2.5 shadow-sm hover:border-slate-300 transition-colors">
                                    <div className="flex flex-col gap-0.5">
                                        ...
                                        <button
                                            onClick={() => movePageOrder(index, 'down')} disabled={index === filteredPages.length - 1}
                                            className={`p-1 rounded-md ${index === filteredPages.length - 1 ? 'text-slate-200' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700'}`}
                                        >
                                            <ArrowDown size={14} />
                                        </button>
                                    </div>
                                    <div className="flex-1 min-w-0 text-left">
                                        ...
                                        <div className="text-xs text-slate-500">{page.page_type}</div>
                                    </div>
