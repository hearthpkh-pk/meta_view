...
export default function PagesView({ showNotification }: { showNotification: (msg: string, type?: 'success' | 'error') => void }) {
    ...
    const syncPagesFromTokens = useAppStore((state) => state.syncPagesFromTokens);
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="p-5 flex flex-col sm:flex-row gap-4 items-end bg-slate-50/50">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                        <button
                            onClick={async () => {
                                const res = await syncPagesFromTokens();
                                if (res.success) showNotification(`ซิงค์ข้อมูลสำเร็จ: ${res.count} เพจ`, 'success');
                                else showNotification(`ผิดพลาด: ${res.error}`, 'error');
                            }}
                            disabled={isLoading}
                            className="bg-slate-800 hover:bg-slate-900 text-white font-medium py-2.5 px-6 rounded-xl transition-all shadow-sm hover:shadow w-full sm:w-auto flex items-center justify-center gap-2 h-[42px] disabled:opacity-50"
                        >
                            <Flag size={18} /> {isLoading ? 'กำลังซิงค์...' : 'ดึงข้อมูลจาก Facebook'}
                        </button>
                        <button
