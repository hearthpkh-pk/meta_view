        ...
        <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">ทั้งหมด (Total Pages)</p>
                    <h3 className="text-2xl font-bold text-slate-900">{filteredPages.length}</h3>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-emerald-100 shadow-sm">
                    <p className="text-xs font-bold text-emerald-600 uppercase tracking-wider mb-1">ปกติ (Healthy)</p>
                    <h3 className="text-2xl font-bold text-slate-900">{filteredPages.filter(p => p.status.includes('ปกติ')).length}</h3>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-amber-100 shadow-sm">
                    <p className="text-xs font-bold text-amber-600 uppercase tracking-wider mb-1">แจ้งเตือน (Warning)</p>
                    <h3 className="text-2xl font-bold text-slate-900">{filteredPages.filter(p => p.status.includes('แจ้งเตือน')).length}</h3>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-rose-100 shadow-sm">
                    <p className="text-xs font-bold text-rose-600 uppercase tracking-wider mb-1">จำกัด/มีปัญหา (Problem)</p>
                    <h3 className="text-2xl font-bold text-slate-900">{filteredPages.filter(p => p.status.includes('จำกัด') || p.status.includes('ปัญหา')).length}</h3>
                </div>
            </div>
